...
// ================================================================
// ===       Face detection libraries and definitions           ===
// ================================================================
...
static void draw_face_boxes(fb_data_t *fb, std::list<dl::detect::result_t> *results, int face_id) {
  ...
  if (fb->bytes_per_pixel == 2) color = ((color >> 8) & 0xF800) | ((color >> 3) & 0x07E0) | (color & 0x001F);
  ...
  for (std::list<dl::detect::result_t>::iterator prediction = results->begin(); prediction != results->end(); prediction++, i++) {
    ...
    //The center coordinates of the bounding box are calculated and printed to the Serial port. Frame size is set to 160x120
    int Center_X = (x + (w / 2)) - 80;
    int Center_Y = (y + (h / 2)) - 60;
    Serial.printf("X%d\tY%d\n", Center_X, Center_Y);  // This is the line that sends commands to the Launcher Base
  }
}
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (true) {
    ...
    if (!fb) {
      ...
      if (fb->width > 159) {  // check to see if image buffer is full/ready

        HumanFaceDetectMSR01 s1(0.05F, 0.3F, 1, 0.5F);                                                                     // configures face detection parameterts
        std::list<dl::detect::result_t> &results = s1.infer((uint16_t *)fb->buf, { (int)fb->height, (int)fb->width, 3 });  // conducts face detection

        if (results.size() > 0) {  // check if face is detected and processes box data
          fb_data_t rfb;
          rfb.width = fb->width;
          ...
          draw_face_boxes(&rfb, &results, 0);  // Draw boxes around face
        }
...
// Checks for valid commands and sends to UART port for control of Launcher Base
  int res = 0;
  int LED_Max = 50;
  int StepSize = 10; // This is how far each command will request to move the Launcher Base
